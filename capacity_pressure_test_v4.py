"""
保存位置: Anla/capacity_pressure_test_v4.py
...
用法:
    python -m Anla.capacity_pressure_test_v4
    python -m Anla.capacity_pressure_test_v4 --configs A
    python -m Anla.capacity_pressure_test_v4 --configs A --loss elegant   # 回退到 v3
"""
...
from Anla.boltzmann_elegant import compute_boltzmann_elegant_loss_and_force
...
def quick_tda_audit(z, k=6):
    try:
        from Anla.validate_tda_loops import compute_geodesic_persistence
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str,
        default=os.path.join("Logs", "capacity_pressure_test_v4"),
        help="输出根目录"
    )
    parser.add_argument(
