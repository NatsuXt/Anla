...
用法 (三种方式):
    ...
    1. CLI — 从 checkpoint 加载模型运行诊断:
       python -m Anla.diagnostic_probe --config A
       python -m Anla.diagnostic_probe --config A --checkpoint path/to/best_checkpoint.pth

    2. 训练结束后内联调用 (推荐):
       from Anla.diagnostic_probe import run_diagnostic_with_model, print_diagnostic_report
       report = run_diagnostic_with_model(model, gen, cfg, device)
       print_diagnostic_report(report)

    3. 训练循环内逐步调用 (轻量):
       from Anla.diagnostic_probe import probe_embedding_stability
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="示例:\n"
               ...
               "  python -m Anla.diagnostic_probe --config A --output-dir Logs/capacity_pressure_test\n",
    ...
    parser.add_argument(
        "--output-dir", default="Logs/capacity_pressure_test",
    ...
    from Anla.capacity_pressure_test import (
        EXPERIMENT_CONFIGS,
